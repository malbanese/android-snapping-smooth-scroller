...
public class SnappingSmoothScroller extends LinearSmoothScroller {
    ...
    private float[] calculateChildSnapLocations(View child, float[] snapResult, int direction) {
        ...
        if(layoutManager != null) {
            ...
            if(direction == DIRECTION_X && layoutManager.canScrollHorizontally()) {
                ...
                snapResult[0] = calculateSnapLocation(start, end, mChildSnap[0]);
                snapResult[1] = calculateSnapLocation(start, end, mChildSnap[1]);
            }
    ...
    private float[] calculateParentSnapLocations(float[] snapResult, int direction) {
        ...
        if(layoutManager != null) {
            ...
            snapResult[0] = calculateSnapLocation(start, end, mParentSnap[0]);
            snapResult[1] = calculateSnapLocation(start, end, mParentSnap[1]);
        }
    ...
    protected float calculateSpeedPerPixel(DisplayMetrics displayMetrics) {
        return mMSPerInch / displayMetrics.densityDpi;
    }
    ...
    protected void onTargetFound(View targetView, RecyclerView.State state, Action action) {
        ...
        if(dx != 0 || dy != 0) {
            action.update(-dx, -dy, 500, mTargetFoundInterpolator);
        }
    }
}
